...
#include "src/dimensity_profiler_cmd/dimensity_profiler_cmd.h"
...
std::atomic<perfetto::DimensityProfilerCmd*> g_perfetto_cmd;
...
void DimensityProfilerCmd::PrintUsage(const char* argv0) {
  fprintf(stderr, R"(
Usage: %s
...
std::optional<int> DimensityProfilerCmd::ParseCmdlineAndMaybeDaemonize(int argc,
                                                              char** argv) {
#if !PERFETTO_BUILDFLAG(PERFETTO_OS_WIN)
  ...
  for (;;) {
    ...
    if (option == 'c') {
      ...
      if (strcmp(optarg, "-") == 0) {
        ...
        // the new DimensityProfilerCmd instance, we need to know upfront trace config
...
void DimensityProfilerCmd::NotifyBgProcessPipe(BgProcessStatus status) {
#if !PERFETTO_BUILDFLAG(PERFETTO_OS_WIN)
...
DimensityProfilerCmd::BgProcessStatus DimensityProfilerCmd::WaitOnBgProcessPipe() {
#if !PERFETTO_BUILDFLAG(PERFETTO_OS_WIN)
...
int DimensityProfilerCmd::ConnectToServiceRunAndMaybeNotify() {
...
int DimensityProfilerCmd::ConnectToServiceAndRun() {
...
#endif
  PERFETTO_LOG("before ConsumerIPCClient::Connect");
  consumer_endpoint_ =
      ConsumerIPCClient::Connect(GetConsumerSocket(), this, &task_runner_);
...
void DimensityProfilerCmd::OnConnect() {
  ...
  if (clone_tsid_.has_value()) {
    task_runner_.PostDelayedTask(std::bind(&DimensityProfilerCmd::OnTimeout, this),
                                 kCloneTimeoutMs);
...
void DimensityProfilerCmd::OnDisconnect() {
  if (connected_) {
...
void DimensityProfilerCmd::CheckTraceDataTimeout() {
  ...
  task_runner_.PostDelayedTask(
      std::bind(&DimensityProfilerCmd::CheckTraceDataTimeout, this),
      kOnTraceDataTimeoutMs);
}

void DimensityProfilerCmd::OnTraceData(std::vector<TracePacket> packets, bool has_more) {
...
void DimensityProfilerCmd::OnTracingDisabled(const std::string& error) {
  ReadbackTraceDataAndQuit(error);
}

void DimensityProfilerCmd::ReadbackTraceDataAndQuit(const std::string& error) {
  if (!error.empty()) {
...
bool DimensityProfilerCmd::OpenOutputFile() {
  base::ScopedFile fd;
...
void DimensityProfilerCmd::SetupCtrlCSignalHandler() {
  ...
  base::InstallCtrlCHandler([] {
    if (DimensityProfilerCmd* main_thread = g_perfetto_cmd.load())
...
void DimensityProfilerCmd::OnDetach(bool success) {
  if (!success) {
...
void DimensityProfilerCmd::OnAttach(bool success, const TraceConfig& trace_config) {
  if (!success) {
...
void DimensityProfilerCmd::OnTraceStats(bool /*success*/,
                               const TraceStats& /*trace_config*/) {
  // TODO(eseckler): Support GetTraceStats().
}

void DimensityProfilerCmd::OnSessionCloned(const OnSessionClonedArgs& args) {
...
void DimensityProfilerCmd::PrintServiceState(bool success,
...
void DimensityProfilerCmd::OnObservableEvents(
...
void DimensityProfilerCmd::OnCloneSnapshotTriggerReceived(TracingSessionID tsid,
                                                 std::string trigger_name) {
  std::string cmdline;
  cmdline.reserve(128);
...
void DimensityProfilerCmd::CloneSessionOnThread(
  ...
  // We should never end up in a state where each secondary DimensityProfilerCmd
  ...
  if (snapshot_threads_.empty() || thread_mode == kNewThreadPerRequest) {
    // The destructor of the main-thread's DimensityProfilerCmdMain will destroy and
    // join the threads that we are crating here.
    snapshot_threads_.emplace_back(
        base::ThreadTaskRunner::CreateAndStart("snapshot"));
  }

  // We need to pass a copy of the trace config to the new DimensityProfilerCmd instance
  ...
  snapshot_threads_.back().PostTask(
      [tsid, cmdline, trace_config_copy, trigger_name, on_clone_callback] {
        ...
        // the underlying string storage that gets std::moved to DimensityProfilerCmd.
        ...
        perfetto::DimensityProfilerCmd cmd;
...
void DimensityProfilerCmd::CloneAllBugreportTraces(
    bool success,
  ...
  // sequence of tasks that DimensityProfilerCmd involves, there is no race here.
  ...
  // 2) Internally DimensityProfilerCmd creates its own UnixTaskRunner, which creates
...
void DimensityProfilerCmd::LogUploadEvent(PerfettoStatsdAtom atom) {
  if (!statsd_logging_)
    return;
...
void DimensityProfilerCmd::LogTriggerEvents(
...
int PERFETTO_EXPORT_ENTRYPOINT DimensityProfilerCmdMain(int argc, char** argv) {
  perfetto::DimensityProfilerCmd cmd;
